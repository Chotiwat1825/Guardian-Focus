document.addEventListener('DOMContentLoaded', () => {
    ...
    chrome.storage.local.get(['isEnabled', 'allowedSites', 'isStrictMode', 'isBreakMode'], (data) => {
        ...
        const isBreakMode = data.isBreakMode || false; // 🌟 ดึงค่าพักเบรก

        masterToggle.checked = isEnabled;
        updateMasterUI(isEnabled, isBreakMode);

        strictModeToggle.checked = data.isStrictMode || false;

        let allowedSites = data.allowedSites || [DEFAULT_SITE, LOCAL_SITE, LOCAL_IP_SITE];
        renderSiteList(allowedSites);

        // ดักจับการเปลี่ยนแปลงแบบ Real-time เวลาหน้าเว็บเปลี่ยนโหมด
        chrome.storage.onChanged.addListener((changes, namespace) => {
            if (namespace === 'local' && changes.isBreakMode !== undefined) {
                updateMasterUI(masterToggle.checked, changes.isBreakMode.newValue);
            }
        });
            ...
            if (tabs.length > 0 && tabs[0].url) {
                try {
                    ...
                    if (currentDomain === 'newtab' || url.protocol === 'chrome:' || url.protocol === 'edge:') {
                        ...
                        siteStatusBox.style.background = '#F3F4F6';
    ...
    // 🌟 อัปเดต UI ให้สอดคล้องกับโหมดพักเบรก
    function updateMasterUI(isOn, isBreakMode = false) {
        if (isBreakMode) {
            statusText.textContent = "☕️ พักเบรก (ปลดล็อก)";
            statusText.style.color = "#F59E0B"; // สีส้มอำพัน
    ...
    masterToggle.addEventListener('change', () => {
        ...
        chrome.storage.local.get(['isBreakMode'], (data) => {
            updateMasterUI(isOn, data.isBreakMode || false);
        });
    });

    strictModeToggle.addEventListener('change', () => {
        chrome.storage.local.set({ isStrictMode: strictModeToggle.checked });
    });
